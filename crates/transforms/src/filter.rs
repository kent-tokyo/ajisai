    ...
    pub fn evaluate(&self, row: &Row) -> bool {
        match self {
            Condition::IsNull { field } => {
                row.get(field).map(|v| v.is_null()).unwrap_or(true)
            }
...
impl FilterRows {
    ...
    pub fn from_json(value: serde_json::Value) -> Result<Box<dyn Transform>> {
        let config: FilterRowsConfig = serde_json::from_value(value)
...
mod tests {
    ...
    fn filter_eq() {
        ...
        let cond = Condition::Eq { field: "name".into(), value: "Alice".into() };
        assert!(cond.evaluate(&row));
        let cond2 = Condition::Eq { field: "name".into(), value: "Bob".into() };
    ...
    fn filter_gt() {
        let row = make_row("Alice", 30);
        let cond = Condition::Gt { field: "age".into(), value: "20".into() };
        assert!(cond.evaluate(&row));
        let cond2 = Condition::Gt { field: "age".into(), value: "40".into() };
        assert!(!cond2.evaluate(&row));
    }
}
