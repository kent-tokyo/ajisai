...
pub enum Operation {
    ...
    Subtract { a: Operand, b: Operand },
    Multiply { a: Operand, b: Operand },
    Divide   { a: Operand, b: Operand },
    Modulo   { a: Operand, b: Operand },
    // String
    Concat   { parts: Vec<Operand> },
    Upper    { field: String },
    Lower    { field: String },
    Trim     { field: String },
    Length   { field: String },
    ...
    ToInteger { field: String },
    ToFloat   { field: String },
    // Conditional
    IfNull   { field: String, default: Operand },
    ...
    pub fn evaluate(&self, row: &Row) -> Value {
        match self {
            ...
            Operation::Modulo { a, b } => {
                match (&a.resolve(row), &b.resolve(row)) {
            ...
            Operation::Upper  { field } => {
                row.get(field).and_then(|v| v.as_str().map(|s| Value::Str(s.to_uppercase())))
                    .unwrap_or(Value::Null)
            }
            Operation::Lower  { field } => {
                row.get(field).and_then(|v| v.as_str().map(|s| Value::Str(s.to_lowercase())))
                    .unwrap_or(Value::Null)
            }
            Operation::Trim   { field } => {
                row.get(field).and_then(|v| v.as_str().map(|s| Value::Str(s.trim().to_owned())))
                    .unwrap_or(Value::Null)
            }
            Operation::Length { field } => {
                row.get(field).and_then(|v| v.as_str().map(|s| Value::Int(s.len() as i64)))
                    .unwrap_or(Value::Null)
            }
            Operation::Substring { field, start, length } => {
                row.get(field).and_then(|v| v.as_str()).map(|s| {
            ...
            Operation::Replace { field, from, to } => {
                row.get(field).and_then(|v| v.as_str())
                    .map(|s| Value::Str(s.replace(from.as_str(), to.as_str())))
                    .unwrap_or(Value::Null)
            }
            Operation::ToString  { field } => {
                row.get(field).map(|v| Value::Str(v.to_display_string())).unwrap_or(Value::Null)
            }
            Operation::ToInteger { field } => {
                row.get(field).and_then(|v| to_f64(v)).map(|f| Value::Int(f as i64))
                    .unwrap_or(Value::Null)
            }
            Operation::ToFloat { field } => {
                row.get(field).and_then(|v| to_f64(v)).map(Value::Float)
                    .unwrap_or(Value::Null)
            }
            Operation::IfNull { field, default } => {
                match row.get(field) {
                    Some(v) if !v.is_null() => v.clone(),
                    _                       => default.resolve(row),
                }
            }
            Operation::Coalesce { fields } => {
                fields.iter()
...
impl CalculatorStep {
    ...
    pub fn from_json(value: serde_json::Value) -> Result<Box<dyn Transform>> {
        let config: CalculatorConfig = serde_json::from_value(value)
...
impl Transform for CalculatorStep {
    fn name(&self) -> &str { "CalculatorStep" }
...
mod tests {
        ...
        let config = CalculatorConfig {
            calculations: vec![Calculation {
                ...
                operation:  Operation::Multiply {
                    a: Operand::Field { field: "price".into() },
                    b: Operand::Field { field: "quantity".into() },
                },
            }],
        };
    ...
    async fn trim_and_upper() {
        let config = CalculatorConfig {
            calculations: vec![
                Calculation {
                    ...
                    operation: Operation::Trim { field: "name".into() },
                },
                Calculation {
                    field: "uppername".into(),
                    value_type: "String".into(),
                    operation: Operation::Upper { field: "name".into() },
                },
            ],
        };
        let mut calc = CalculatorStep::new(config);
        ...
        assert_eq!(out[0].get("uppername"), Some(&Value::Str("  HELLO  ".into())));
    }
}
