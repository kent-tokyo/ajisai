...
impl AddConstants {
    pub fn new(config: AddConstantsConfig) -> Self {
        Self { config, output_schema: None, const_values: Vec::new() }
    }

    pub fn from_json(value: serde_json::Value) -> Result<Box<dyn Transform>> {
        let config: AddConstantsConfig = serde_json::from_value(value)
            .map_err(|e| AjisaiError::Config(e.to_string()))?;
        Ok(Box::new(Self::new(config)))
    }

    fn parse_constant(cf: &ConstantField) -> Result<(Field, Value)> {
        let (vt, value) = match cf.value_type.as_str() {
            "Integer" => {
                let n = cf.value.parse::<i64>()
                    .map_err(|_| AjisaiError::Parse(format!("'{}' is not an Integer", cf.value)))?;
                (ValueType::Integer, Value::Int(n))
            }
            "Float" => {
                let f = cf.value.parse::<f64>()
                    .map_err(|_| AjisaiError::Parse(format!("'{}' is not a Float", cf.value)))?;
