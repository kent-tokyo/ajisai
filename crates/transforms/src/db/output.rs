...
fn validate_sql_identifier(name: &str) -> Result<()> {
    ...
    if name.chars().next().map(|c| c.is_ascii_digit()).unwrap_or(false) {
        return Err(AjisaiError::Config(format!(
            "SQL identifier '{}' must not start with a digit", name
        )));
    }
    if !name.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') {
        return Err(AjisaiError::Config(format!(
            "SQL identifier '{}' contains invalid characters \
             (only ASCII letters, digits, and underscores are allowed)", name
        )));
    }
    Ok(())
}
...
impl TableOutput {
    pub fn new(config: TableOutputConfig) -> Self {
        Self { config, pool: None, buffer: Vec::new() }
    }

    pub fn from_json(value: serde_json::Value) -> Result<Box<dyn Transform>> {
        let config: TableOutputConfig = serde_json::from_value(value)
    ...
    async fn flush(&mut self) -> Result<()> {
        ...
        let cols: Vec<&str> = first.schema.fields.iter().map(|f| f.name.as_str()).collect();
        for col in &cols {
