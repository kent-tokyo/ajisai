...
impl Action for PipelineAction {
    ...
    async fn execute<'a>(&'a mut self, ctx: &'a ExecutionContext) -> ActionResult {
        ...
        let pipeline = match crate::parser::pipeline::hop_pipeline_to_ajisai(hop_pipeline, &registry) {
            Ok(p) => p,
            Err(e) => return ActionResult::err(e.to_string()),
        };
...
pub fn hop_workflow_to_ajisai(
    ...
    for action in hw.actions {
        let pipeline_file = action.attributes
            .get("filename")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        ...
        let path = if pipeline_file.is_empty() {
            ...
            if p.is_absolute() { p.to_owned() } else { base_dir.join(p) }
        };

        let action_name = action.name.clone();
        workflow.add_action(
            action_name,
            Box::new(PipelineAction { name: action.name, pipeline_path: path }),
        );
    }
    ...
    for hop in hw.hops {
        ...
        workflow.add_hop(hop.from, hop.to, evaluation, hop.unconditional.unwrap_or(false));
    }
